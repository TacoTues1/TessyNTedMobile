        ...
        >
            ...
            <View style={styles.header}>
                <View style={styles.headerActions}>
                    <TouchableOpacity onPress={openEmailModal} style={styles.iconBtnTrans}>
                        <Ionicons name="mail-outline" size={24} color="white" />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => router.push('/properties/new' as any)} style={styles.iconBtnWhite}>
            ...
            <View style={styles.sectionContainer}>
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}>
                    <TouchableOpacity style={[styles.card, { flex: 1, padding: 15 }]} onPress={() => router.push('/maintenance' as any)}>
                        <Text style={styles.miniTitle}>Maintenance</Text>
                        <Text style={{ fontSize: 24, fontWeight: 'bold' }}>{tasks.maintenance.length}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.card, { flex: 1, padding: 15 }]} onPress={() => router.push('/payments' as any)}>
                        <Text style={styles.miniTitle}>Payments</Text>
                        <Text style={{ fontSize: 24, fontWeight: 'bold' }}>{tasks.payments.length}</Text>
                    </TouchableOpacity>
                </View>
            </View>
            ...
            <View style={styles.sectionContainer}>
                ...
                {properties.map((property) => {
                    ...
                    return (
                        <TouchableOpacity key={property.id} style={styles.propCard} onPress={() => router.push(`/properties/edit/${property.id}` as any)}>
                            ...
                            <View style={styles.propContent}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                    ...
                                    <Text style={styles.propPrice}>₱{(property.price / 1000).toFixed(0)}k</Text>
                                </View>
                                <Text style={styles.propAddress}>{property.city}</Text>
                                <View style={{ marginTop: 15 }}>
                                    {occupancy ? (
                                        <View style={styles.occupantRow}>
                                            <Text style={styles.occupantName}>{occupancy.tenant?.first_name} {occupancy.tenant?.last_name}</Text>
                                            <TouchableOpacity onPress={() => openEndContractModal(occupancy)} style={styles.btnEnd}>
                                                <Text style={styles.btnEndText}>End Contract</Text>
                                            </TouchableOpacity>
                                        </View>
                                    ) : (
                                        <TouchableOpacity style={styles.assignBtn} onPress={() => openAssignModal(property)}>
            ...
            <Modal visible={showEmailModal} animationType="slide" presentationStyle="pageSheet">
                <View style={styles.fullScreenModal}>
                    <View style={styles.modalHeader}>
                        <Text style={styles.modalTitle}>Message Tenants</Text>
                        <TouchableOpacity onPress={() => setShowEmailModal(false)}><Text style={{ color: 'blue', fontSize: 16 }}>Cancel</Text></TouchableOpacity>
                    </View>
                    <ScrollView style={{ padding: 20 }}>
                        <Text style={styles.label}>Recipients</Text>
                        <TouchableOpacity onPress={() => setShowTenantDropdown(!showTenantDropdown)} style={styles.dropdownTrigger}>
                            <Text style={{ flex: 1 }}>{selectedTenants.length === 0 ? 'Select tenants...' : `${selectedTenants.length} selected`}</Text>
                            <Ionicons name={showTenantDropdown ? "chevron-up" : "chevron-down"} size={20} />
                        </TouchableOpacity>

                        {showTenantDropdown && (
                            <View style={styles.dropdownContent}>
                                <TouchableOpacity onPress={selectAllTenants} style={styles.dropdownItem}>
                                    <Text style={{ fontWeight: 'bold' }}>Select All ({allTenants.length})</Text>
                                </TouchableOpacity>
                                {allTenants.map(t => (
                                    <TouchableOpacity key={t.id} onPress={() => toggleTenantSelection(t.id)} style={styles.dropdownItem}>
                                        <Ionicons name={selectedTenants.includes(t.id) ? "checkbox" : "square-outline"} size={20} color={selectedTenants.includes(t.id) ? "black" : "#ccc"} />
                                        <Text style={{ marginLeft: 10 }}>{t.name} ({t.property})</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        )}
                        ...
                        <TextInput style={styles.input} value={emailSubject} onChangeText={setEmailSubject} placeholder="Important Notice..." />
                        ...
                        <TouchableOpacity
                            ...
                            style={[styles.btnFull, { backgroundColor: 'black', marginTop: 20 }]}
                        >
                            {sendingEmail ? <ActivityIndicator color="white" /> : <Text style={styles.btnTextWhite}>Send Notification</Text>}
                        </TouchableOpacity>
                    </ScrollView>
                </View>
            </Modal>
...
const styles = StyleSheet.create({
    ...
    // Header
    header: { backgroundColor: '#111827', padding: 25, paddingBottom: 40, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 },
    welcomeText: { color: '#9ca3af', fontSize: 14, fontWeight: '600' },
    nameText: { color: 'white', fontSize: 28, fontWeight: 'bold' },
    headerActions: { flexDirection: 'row', gap: 10, position: 'absolute', right: 20, top: 30 },
    iconBtnTrans: { width: 45, height: 45, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.1)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
    iconBtnWhite: { width: 45, height: 45, borderRadius: 12, backgroundColor: 'white', alignItems: 'center', justifyContent: 'center' },

    // Grid
    gridContainer: { flexDirection: 'row', flexWrap: 'wrap', padding: 15, marginTop: -30 },
    ...
    propStatus: { position: 'absolute', top: 12, left: 12, backgroundColor: 'white', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
    propStatusText: { fontSize: 10, fontWeight: 'bold', textTransform: 'uppercase' },
    ...
    propAddress: { fontSize: 12, color: '#666', marginTop: 2 },
    ...
    fullScreenModal: { flex: 1, backgroundColor: 'white', marginTop: 50, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
